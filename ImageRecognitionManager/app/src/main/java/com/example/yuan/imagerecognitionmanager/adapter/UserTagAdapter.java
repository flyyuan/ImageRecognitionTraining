...
public class UserTagAdapter extends SwipeMenuAdapter<UserTagAdapter.DefaultViewHolder> {
    ...
    public void onBindViewHolder(UserTagAdapter.DefaultViewHolder holder, int position) {
        ...
        if (labelsByUserId.getSuccLabels()!=null && labelsByUserId.getErrorLabels() != null){
        holder.tagExpTv1.setText("标注成功："+labelsByUserId.getSuccLabels() + "\n"
                + "标注失败："+labelsByUserId.getErrorLabels());}
        else if (labelsByUserId.getSuccLabels()==null){
            holder.tagExpTv1.setText("标注失败："+labelsByUserId.getErrorLabels());
        }else {
            holder.tagExpTv1.setText("标注成功："+labelsByUserId.getSuccLabels());
        }
        ...
        //加载标签图片
        Glide.with(mContext)
                .load("http://39.108.69.214:8080/pic/image/"+labelsByUserId.getUrl())
